...
class _HomePageState extends State<HomePage> {
  ...
  Future<void> _onNavTap(int index, bool isAdmin) async {
...
// ===========================================================================
//  المفضّلة — الأخبار التي أعجب بها المستخدم الحالي
// ===========================================================================
...
class FavoritesPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: StreamBuilder<QuerySnapshot<Map<String, dynamic>>>(
        stream: Api.db
            .collectionGroup(AppConfig.likesSubcollection)
            .where('uid', isEqualTo: Api.uid)
            .snapshots(),
        builder: (context, likesSnap) {
          if (likesSnap.hasError) {
            return const Center(
              child: Padding(
                padding: EdgeInsets.all(24),
                child: Text(
                  'تعذّر تحميل المفضّلة. إن ظهر هذا أول مرة، افتح رسالة الخطأ '
                  'في وحدة التحكم واضغط رابط إنشاء الفهرس، ثم أعد المحاولة.',
                  textAlign: TextAlign.center,
                  style: TextStyle(color: AppConfig.textSoft),
                ),
              ),
          ...
          final postIds = likesSnap.data!.docs
          ...
          if (postIds.isEmpty) {
            return const Center(
              child: Text('لم تُعجب بأي خبر بعد',
                  style: TextStyle(color: AppConfig.textSoft)),
            );
          }

          // حد Firestore: 30 قيمة كحد أقصى لكل استعلام whereIn
          final chunk = postIds.take(30).toList();

          return FutureBuilder<QuerySnapshot<Map<String, dynamic>>>(
            future: Api.posts.where(FieldPath.documentId, whereIn: chunk).get(),
            builder: (context, postsSnap) {
              if (!postsSnap.hasData) {
                return const Center(child: CircularProgressIndicator());
              }
              final posts = postsSnap.data!.docs.map(Post.fromDoc).toList()
              ...
              return ListView.separated(
                ...
                itemBuilder: (context, i) => Center(
                  child: ConstrainedBox(
                    constraints: const BoxConstraints(maxWidth: 680),
                    child: PostCard(post: posts[i], isAdmin: false),
                  ),
                ),
              );
            },
